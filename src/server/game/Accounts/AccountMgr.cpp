...
void AccountMgr::LoadRBAC()
{
    ...
    result = LoginDatabase.PQuery("SELECT secId, permissionId FROM rbac_default_permissions WHERE (realmId = %u OR realmId = -1) ORDER BY secId ASC", sRealmHandle->Index);
    if (!result)
    {
