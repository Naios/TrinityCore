...
{
    return ObjectGuid(uint64(uint64(type) << 58 | uint64(sRealmHandle->Index) << 42), counter);
}

ObjectGuid ObjectGuid::MapSpecific(HighGuid type, uint8 subType, uint16 mapId, uint32 serverId, uint32 entry, LowType counter)
{
    return ObjectGuid(uint64((uint64(type) << 58) | (uint64(sRealmHandle->Index & 0x1FFF) << 42) | (uint64(mapId & 0x1FFF) << 29) | (uint64(entry & 0x7FFFFF) << 6) | (uint64(subType) & 0x3F)),
