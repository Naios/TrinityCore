    ...
    : public ScriptRegistrySwapHookBase
{
    ...
    static void UnloadScriptHelper(Creature* creature)
    {
    ...
    static void UnloadScriptHelper(GameObject* gameobject)
    {
        gameobject->AI()->Reset();
    ...
    static void LoadScriptHelper(Creature* creature)
    {
        ASSERT(!creature->AI(),
    ...
    static void LoadScriptHelper(GameObject* gameobject)
    {
...
public:
    void BeforeReleaseContext(std::string const& context) final override
    {
        ...
        // Destroy every AI which objects script id is contained in ids_removed_
        auto evaluator = [&](std::unordered_map<ObjectGuid, ObjectType*>& objects)
        {
            // Calls the specialized unloader for gameobjects and creatures
            for (auto const& object : objects)
                if (ids_to_remove.find(object.second->GetScriptId()) != ids_to_remove.end())
                    UnloadScriptHelper(object.second);
        };
    ...
    void BeforeSwapContext() final override
    {
        ...
        // Loads every AI which objects script id is contained in ids_removed_
        auto evaluator = [&](std::unordered_map<ObjectGuid, ObjectType*>& objects)
        {
            // Calls the specialized loader and unloader for gameobjects and creatures
            for (auto const& object : objects)
                if (ids_removed_.find(object.second->GetScriptId()) != ids_removed_.end())
                {
                    // Overwrite existing (default) AI's which are replaced by a new script
                    if (object.second->AI())
                        UnloadScriptHelper(object.second);

                    LoadScriptHelper(object.second);
                }
        };
...
void LazyUnloadedScriptObject::AcquireReference(const char* scriptname)
{
...
InstanceMapScript::InstanceMapScript(const char* name, uint32 mapId)
    : LazyUnloadedScriptObject(name), MapScript<InstanceMap>(mapId)
{
