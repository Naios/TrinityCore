...
{
    return "TDB_full_6.00_2014_10_19.sql";
}

template<>
std::string DBUpdater<WorldDatabaseConnection>::GetDownloadLink()
{
    return "https://github.com/TrinityCore/TrinityCore/releases/download/TDB6.00/TDB_full_6.00_2014_10_19.7z";
}
...
template<class T>
std::string DBUpdater<T>::GetDownloadLink()
{
    return "";
}
...
{
    // Use std::cout here to avoid bugs due to possible async logging
    std::cout << "Database \"" << pool.GetConnectionInfo()->database << "\" does not exist, do you want to create it? [yes (default) / no]: ";
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    {
        TC_LOG_ERROR("sql.updates", "DBUpdater: Given source directory %s does not exist, skipped!", sourceDirectory.c_str());
        return false;
    }

    auto const apply = [&](std::string const& query) { DBUpdater<T>::Apply(pool, query); };
    auto const applyFile = [&](Path const& file) { DBUpdater<T>::ApplyFile(pool, file); };
    auto const retrieve = [&](std::string const& query) { return DBUpdater<T>::Retrieve(pool, query); };

    UpdateFetcher updateFetcher(sourceDirectory, apply, applyFile, retrieve);
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    if (!exists(base))
    {
        std::string const link = DBUpdater<T>::GetBaseFile();
        if (link.empty())
        {
            TC_LOG_INFO("sql.updates", ">> Base file \"%s\" is missing, try to clone a new source.",
                base.generic_string().c_str(), link.c_str());
        }
        else
        {
            TC_LOG_INFO("sql.updates", ">> File \"%s\" is missing, download it at \"%s\" and place it in your server directory.",
                base.filename().generic_string().c_str(), link.c_str());
        }
    ...
    UpdateFetcher const updateFetcher(DBUpdater<T>::GetSourceDirectory(), apply, applyFile, retrieve);
    return updateFetcher.Populate(base);
}
    ...
    std::string const& password, std::string const& port_or_socket, std::string const& database, Path const& path)
{
    ...
    try
    {
        child c = execute(run_exe(boost::filesystem::absolute(DBUpdater<T>::GetMySqlCli())),
