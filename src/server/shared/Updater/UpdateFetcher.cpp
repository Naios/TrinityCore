...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        {
            TC_LOG_TRACE("sql.updates", "Added locale file \"%s\".", itr->path().filename().string().c_str());
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
        ...
        if (!is_directory(p))
        {
            TC_LOG_ERROR("sql.updates", "DBUpdater: Given update include directory \"%s\" isn't existing, skipped!", p.c_str());
            continue;
        }
        ...
        TC_LOG_TRACE("sql.updates", "Added applied file \"%s\" from remote.", p.filename().string().c_str());
...
uint32 UpdateFetcher::Update(bool const redundancyChecks, bool const allowRehash, bool const archivedRedundancy, bool const cleanDeadReferences) const
{
    ...
    {
        TC_LOG_DEBUG("sql.updates", "Checking update \"%s\"...", availableQuery.first.filename().string().c_str());
        ...
        else
        {
            ...
            {
                TC_LOG_INFO("sql.updates", ">> Reapplying update \"%s\" \'%s\' -> \'%s\' because it changed.", availableQuery.first.filename().string().c_str(),
            ...
            else
            {
                ...
                {
                    TC_LOG_DEBUG("sql.updates", ">> Updating state to \'%s\'.", AppliedFileEntry::StateConvert(availableQuery.second).c_str());
...
{
    // Calculate a MD5 hash based on query content.
    unsigned char digest[MD5_DIGEST_LENGTH];
    MD5((unsigned char*)query->c_str(), query->length(), (unsigned char*)&digest);

    return ByteArrayToHexStr(digest, MD5_DIGEST_LENGTH);
}
...
bool UpdateFetcher::Populate(Path const& base) const
{
    // Update database
