...
/* Coisas a fazer:
Waypoints dos trashs da sala do Akama
Waypoints dos Npcs (Broken) da introducao final
Revisar timers de maioria das creatures
Criar uma formula para a spell de slow
*/
enum Says
{
...
public:
    ...
    struct boss_shade_of_akamaAI : public BossAI
    {
        ...
        {
            if (who->GetGUID() == instance->GetGuidData(DATA_AKAMA_SHADE) && me->IsWithinMeleeRange(who, 5.0f))
            {
...
public:
    ...
    struct npc_akamaAI : public ScriptedAI
    {
        ...
        {
            if (victim->GetGUID() == instance->GetGuidData(DATA_AKAMA_SHADE))
                me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_NONE);
        }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_CHAIN_LIGHTNING:
                        ...
                        events.ScheduleEvent(EVENT_CHAIN_LIGHTNING, randtime(Seconds(10), Seconds(15)));
                        break;
                    case EVENT_DESTRUCTIVE_POISON:
                        DoCastSelf(SPELL_DESTRUCTIVE_POISON);
                        events.ScheduleEvent(EVENT_DESTRUCTIVE_POISON, randtime(Seconds(4), Seconds(5)));
                        break;
...
// Precisa confirmar os tempos (aguardando sniff do gustavo)
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_SPAWN_WAVE_B:
                        ...
                        events.ScheduleEvent(EVENT_SPAWN_WAVE_B, randtime(Seconds(45), Seconds(50)));
                        break;
                    ...
                    case EVENT_SUMMON_ASHTONGUE_DEFENDER: // right
                        ...
                        events.ScheduleEvent(EVENT_SUMMON_ASHTONGUE_DEFENDER, randtime(Seconds(30), Seconds(35)));
                        break;
                    default:
                        break;
                }
...
//Faltando conferir timers (aguardando sniff gustavo)
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_DEBILITATING_STRIKE:
                        ...
                        events.ScheduleEvent(EVENT_DEBILITATING_STRIKE, randtime(Seconds(8), Seconds(16)));
                        break;
                    case EVENT_HEROIC_STRIKE:
                        DoCastSelf(SPELL_HEROIC_STRIKE);
                        events.ScheduleEvent(EVENT_HEROIC_STRIKE, randtime(Seconds(50), Seconds(60)));
                        break;
                    case EVENT_SHIELD_BASH:
                        DoCastVictim(SPELL_SHIELD_BASH);
                        events.ScheduleEvent(EVENT_SHIELD_BASH, randtime(Seconds(8), Seconds(16)));
                        break;
                    case EVENT_WINDFURY:
                        DoCastVictim(SPELL_WINDFURY);
                        events.ScheduleEvent(EVENT_WINDFURY, randtime(Seconds(), Seconds(8)));
                        break;
                    default:
                        break;
                }
...
//Conferir timers com sniff do gustavo
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_DEBILITATING_POISON:
                        ...
                        events.ScheduleEvent(EVENT_DEBILITATING_POISON, randtime(Seconds(14), Seconds(18)));
                        break;
                    case EVENT_EVISCERATE:
                        DoCastVictim(SPELL_EVISCERATE);
                        events.ScheduleEvent(EVENT_EVISCERATE, randtime(Seconds(12), Seconds(16)));
                        break;
                    default:
                        break;
                }
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_RAIN_OF_FIRE:
                        ...
                        events.ScheduleEvent(EVENT_RAIN_OF_FIRE, Seconds(20));
                        break;
                    case EVENT_LIGHTNING_BOLT:
                        DoCastVictim(SPELL_LIGHTNING_BOLT);
                        events.ScheduleEvent(EVENT_LIGHTNING_BOLT, Seconds(15));
                        break;
                    default:
                        break;
                }
...
public:
    ...
    class spell_shade_soul_channel_two_AuraScript : public AuraScript
    {
        ...
        //HACK ALERT !!!!!
        //THIS NEED BE CHANGED SOON !
        void UpdateSpeed()
        {
            Unit* target = GetTarget();
            if (target->GetAuraCount(GetId()) <= 3)
            {
                float moveSpeed;
                moveSpeed = (2.0f - (0.6f * target->GetAuraCount(GetId())));
